...
export const lessons: Lesson[] = [
  makeLesson("school-functions", 0, "Функция, область определения, график", {
    theory: String.raw`Функция — это соответствие, при котором каждому числу $x$ из множества $D$ ставится ровно одно число $y$. Пишут $y=f(x)$. Множество $D$ называют областью определения и обозначают $D(f)$; множество всех значений $y$ — областью значений $E(f)$.

График функции — множество точек плоскости $(x; f(x))$, где $x\in D(f)$. Вертикальная прямая пересекает график не более чем в одной точке: иначе одному $x$ соответствовало бы несколько $y$.

Чтобы найти $D(f)$, исключают те $x$, при которых выражение не определено: знаменатель не может быть равен нулю, под чётным корнем — отрицательное число, аргумент логарифма должен быть положительным.`,
    examples: [
      {
  ...
  makeLesson("school-functions", 1, "Линейная и квадратичная функции", {
    theory: String.raw`Линейная функция задаётся формулой $y=kx+b$. Число $k$ — угловой коэффициент (наклон): при $k>0$ функция возрастает, при $k<0$ убывает. График — прямая; она пересекает ось $Oy$ в точке $(0;b)$.

Квадратичная функция $y=ax^{2}+bx+c$ при $a\neq 0$ имеет график-параболу. Ветви направлены вверх при $a>0$ и вниз при $a<0$. Абсцисса вершины $x_{0}=-\dfrac{b}{2a}$, ордината $y_{0}=f(x_{0})$. Ось симметрии — прямая $x=x_{0}$.

Корни трёхчлена (если есть) — точки пересечения параболы с осью $Ox$; их находят через дискриминант $D=b^{2}-4ac$.`,
    examples: [
      {
  ...
  makeLesson("school-functions", 2, "Преобразования графиков", {
    theory: String.raw`От графика $y=f(x)$ получают новые графики геометрическими преобразованиями.

Параллельный перенос: $y=f(x)+a$ сдвигает график на $a$ вверх (при $a<0$ — вниз); $y=f(x-a)$ сдвигает на $a$ вправо (при $a<0$ — влево).

Отражения: $y=-f(x)$ — симметрия относительно оси $Ox$; $y=f(-x)$ — относительно оси $Oy$.

Растяжения: $y=kf(x)$ при $|k|>1$ растягивает от оси $Ox$; при $0<|k|<1$ сжимает. Функция $y=f(kx)$ при $|k|>1$ сжимает к оси $Oy$.`,
    examples: [
      {
  ...
  makeLesson("school-functions", 3, "Обратная функция", {
    theory: String.raw`Функция $f$ называется обратимой, если разным аргументам соответствуют разные значения (функция взаимно однозначна). Тогда существует обратная функция $f^{-1}$, для которой $f^{-1}(f(x))=x$ и $f(f^{-1}(y))=y$.

Область определения $f^{-1}$ совпадает с областью значений $f$, и наоборот. Графики $y=f(x)$ и $y=f^{-1}(x)$ симметричны относительно прямой $y=x$.

Чтобы найти формулу обратной функции, в равенстве $y=f(x)$ выражают $x$ через $y$ и меняют обозначения: пишут $y=f^{-1}(x)$. Монотонная непрерывная функция на промежутке всегда обратима.`,
    examples: [
      {
  ...
  makeLesson("school-functions", 4, "Показательная функция и уравнения", {
    theory: String.raw`Показательная функция $y=a^{x}$ определена при $a>0$, $a\neq 1$, для всех действительных $x$. Область значений — $(0;+\infty)$. При $a>1$ функция возрастает, при $0<a<1$ убывает. График проходит через точку $(0;1)$, так как $a^{0}=1$.

Основные свойства: $a^{x}\cdot a^{y}=a^{x+y}$, $\dfrac{a^{x}}{a^{y}}=a^{x-y}$, $(a^{x})^{y}=a^{xy}$, $a^{-x}=\dfrac{1}{a^{x}}$.

Показательное уравнение $a^{f(x)}=a^{g(x)}$ при $a>0$, $a\neq 1$ равносильно $f(x)=g(x)$. Если основания различны, их приводят к одному основанию: например, $4=2^{2}$, $8=2^{3}$, $9=3^{2}$, $27=3^{3}$.`,
    examples: [
      {
  ...
  makeLesson("school-functions", 5, "Логарифмы: определение и свойства", {
    theory: String.raw`Логарифм $\log_{a} b$ при $a>0$, $a\neq 1$, $b>0$ — это показатель степени, в которую нужно возвести $a$, чтобы получить $b$: $\log_{a} b=c$ означает $a^{c}=b$. В частности, $\log_{a} a=1$ и $\log_{a} 1=0$. Десятичный логарифм $\lg b=\log_{10} b$, натуральный $\ln b=\log_{e} b$.

Основные тождества: $a^{\log_{a} b}=b$ и $\log_{a}(a^{x})=x$. Свойства: $\log_{a}(xy)=\log_{a} x+\log_{a} y$, $\log_{a}\dfrac{x}{y}=\log_{a} x-\log_{a} y$, $\log_{a}(x^{p})=p\log_{a} x$ (при допустимых $x,y$).

Формула перехода к другому основанию: $\log_{a} b=\dfrac{\log_{c} b}{\log_{c} a}$. Следствие: $\log_{a} b=\dfrac{1}{\log_{b} a}$.`,
    examples: [
      {
  ...
  makeLesson("school-functions", 6, "Логарифмические уравнения", {
    theory: String.raw`Простейшее логарифмическое уравнение $\log_{a} f(x)=b$ при $a>0$, $a\neq 1$ равносильно $f(x)=a^{b}$ при условии $f(x)>0$ (оно выполняется автоматически, если $a^{b}>0$).

Уравнение $\log_{a} f(x)=\log_{a} g(x)$ равносильно системе $f(x)=g(x)$, $f(x)>0$ (тогда и $g(x)>0$). Перед потенцированием полезно свернуть сумму и разность логарифмов: $\log_{a} f+\log_{a} g=\log_{a}(fg)$.
...
Обязательно проверяют область определения: аргументы логарифмов положительны, основания удовлетворяют $a>0$, $a\neq 1$. Посторонние корни отбрасывают.`,
  ...
  makeLesson("school-functions", 7, "Показательные и логарифмические неравенства", {
    theory: String.raw`Монотонность основания задаёт направление неравенства. Если $a>1$, то $a^{f}>a^{g}$ равносильно $f>g$; если $0<a<1$, знак меняется: $a^{f}>a^{g}$ равносильно $f<g$.

Для логарифмов с основанием $a>1$ неравенство $\log_{a} f>\log_{a} g$ равносильно $f>g$ при $f>0$, $g>0$. Если $0<a<1$, знак снова меняется. Простейшее $\log_{a} f>b$ при $a>1$ даёт $f>a^{b}$ (и $f>0$).

Всегда учитывают область определения: для логарифма аргумент положителен. Показательная функция принимает только положительные значения, поэтому сравнение $a^{x}$ с неположительным числом решается отдельно (например, $2^{x}>0$ — тождество).`,
    examples: [
      {
