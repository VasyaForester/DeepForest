...
export const lessons: Lesson[] = [
  ...
  makeLesson("uni-nt", 14, "Линейные сравнения", {
    purpose:
      "Решение $ax\\equiv b\\pmod{m}$ — рабочий навык после определений сравнений и КТО: обратные, системы, криптография, индексы.",
    nonexample: {
      title: "Не всегда одно решение и не всегда можно «сократить» $a$",
      text: "Сравнение $2x\\equiv 2\\pmod{4}$ имеет два решения $x\\equiv 1,3\\pmod{4}$, а не одно. Сокращение на $2$ дало бы $x\\equiv 1\\pmod{2}$, что описывает оба класса, но не эквивалентно $x\\equiv 1\\pmod{4}$. Делить на $a$ можно лишь после учёта $d=(a,m)$.",
    },
    theory:
      "Линейное сравнение $ax\\equiv b\\pmod{m}$ разрешимо тогда и только тогда, когда $d=(a,m)$ делит $b$. В этом случае ровно $d$ решений по модулю $m$: если $x_0$ — частное решение, остальные суть $x=x_0+(m/d)t$, $t=0,1,\\ldots,d-1$. Сведение: разделив сравнение на $d$, получают $(a/d)x\\equiv b/d\\pmod{m/d}$ с $(a/d,m/d)=1$, у которого ровно одно решение по новому модулю. При $(a,m)=1$ существует обратный $a^{-1}\\pmod{m}$, и $x\\equiv a^{-1}b\\pmod{m}$; обратный находят из тождества Безу или возведением в степень $\\varphi(m)-1$ (Эйлер). Системы линейных сравнений с попарно взаимно простыми модулями решаются китайской теоремой об остатках. Если модули не взаимно просты, нужна совместимость на НОД. Однородное $ax\\equiv 0\\pmod{m}$ описывает аннулятор $a$ в кольце $\\mathbb{Z}/m\\mathbb{Z}$.",
    examples: [
      {
        title: "Взаимно простой коэффициент",
        problem: "Решите $3x\\equiv 4\\pmod{7}$.",
        solution:
          "$(3,7)=1\\mid 4$. Обратный к $3$: $3\\cdot 5=15\\equiv 1\\pmod{7}$, поэтому $x\\equiv 20\\equiv 6\\pmod{7}$. Проверка: $3\\cdot 6=18\\equiv 4$.",
      },
      {
        title: "Несколько решений",
        problem: "Решите $4x\\equiv 6\\pmod{10}$. Сколько решений?",
        solution:
          "$(4,10)=2\\mid 6$, решений два. Деление на $2$: $2x\\equiv 3\\pmod{5}$. Обратный к $2$ по модулю $5$ есть $3$, $x\\equiv 9\\equiv 4\\pmod{5}$. По модулю $10$: $x\\equiv 4$ и $x\\equiv 9$.",
      },
    ],
    sample: {
      id: "s",
      type: "open",
      prompt: "Число решений $6x\\equiv 9\\pmod{15}$ равно $(6,15)$, так как $3\\mid 9$. Это",
      accepted: ["3"],
      explanation: "$d=3$ делит $9$.",
      solution:
        "Критерий: $d\\mid b$ влечёт ровно $d$ классов по модулю $m$.",
    },
    problems: [
      {
        id: "p1",
      ...
      },
      {
        id: "p2",
        type: "open",
        prompt: "$5x\\equiv 3\\pmod{7}$. Обратный к $5$ есть $3$, ибо $15\\equiv 1$. Тогда $x\\equiv 9\\equiv$",
        accepted: ["2"],
        explanation: "$5\\cdot 2=10\\equiv 3\\pmod{7}$.",
      },
      {
        id: "p3",
        type: "choice",
        prompt: "Если $d=(a,m)\\mid b$, число решений по модулю $m$ равно",
        options: ["$1$ всегда", "$d$", "$m$", "$0$"],
        answerIndex: 1,
        explanation: "Шаг $m/d$ даёт $d$ классов.",
      },
      {
        id: "p4",
        type: "open",
        prompt: "$2x\\equiv 1\\pmod{4}$ неразрешимо, так как $(2,4)=2$ не делит $1$. Число решений равно",
        accepted: ["0"],
        explanation: "Критерий нарушен.",
      },
      {
        id: "p5",
        type: "open",
        prompt: "При $(a,m)=1$ существует $a^{-1}$. Для $a=3$, $m=8$: $3\\cdot 3=9\\equiv 1$, обратный равен",
        accepted: ["3"],
        explanation: "$3$ обратен самому себе по модулю $8$.",
      },
      {
        id: "p6",
        type: "choice",
        prompt: "Общее решение при разрешимости: $x=x_0+(m/d)t$ с $t=0,\\ldots,d-1$. При $m=12$, $d=3$ шаг равен",
        options: ["$12$", "$4$", "$3$", "$1$"],
        answerIndex: 1,
        explanation: "$m/d=4$.",
      },
  ...
  makeLesson("uni-nt", 15, "Порядок элемента по модулю", {
    purpose:
      "Порядок в $(\\mathbb{Z}/n\\mathbb{Z})^{\\times}$ уточняет теорему Эйлера и готовит первообразные корни: периоды степеней, критерии, дискретный логарифм.",
    nonexample: {
      title: "Порядок определён не для всех $a$ и не равен $\\varphi(n)$ автоматически",
      text: "При $(a,n)>1$ сравнение $a^{k}\\equiv 1\\pmod{n}$ может не иметь места ни при каком $k>0$ (пример: $2^{k}$ чётно, не сравнимо с $1$ по модулю $4$ при $k\\geqslant 1$). Равенство $a^{\\varphi(n)}\\equiv 1$ не означает, что порядок равен $\\varphi(n)$: порядок — наименьший такой показатель и лишь делит $\\varphi(n)$. Так, $2^{3}\\equiv 1\\pmod{7}$, порядок $3$, хотя $\\varphi(7)=6$.",
    },
    theory:
      "Пусть $(a,n)=1$. Порядком $a$ по модулю $n$ называют $\\mathrm{ord}_{n}(a)=\\min\\{k\\in\\mathbb{N}:a^{k}\\equiv 1\\pmod{n}\\}$. Он существует: по Эйлеру $a^{\\varphi(n)}\\equiv 1$, множество показателей непусто. Свойства: $a^{k}\\equiv 1\\pmod{n}$ тогда и только тогда, когда $\\mathrm{ord}_{n}(a)\\mid k$; в частности $\\mathrm{ord}_{n}(a)\\mid\\varphi(n)$. Далее $a^{i}\\equiv a^{j}\\pmod{n}$ $\\Leftrightarrow$ $\\mathrm{ord}\\mid(i-j)$. Формула для степени: $\\mathrm{ord}_{n}(a^{k})=\\mathrm{ord}_{n}(a)/\\gcd(k,\\mathrm{ord}_{n}(a))$. Число элементов порядка $d$ в циклической группе делит $d$ и равно $\\varphi(d)$, если $d$ делит порядок группы. Первообразный корень — элемент порядка $\\varphi(n)$. Критерий порядка $d$: $a^{d}\\equiv 1$ и $a^{d/q}\\not\\equiv 1$ для всех простых $q\\mid d$.",
    examples: [
      {
        title: "Порядок $2$ по модулю $5$",
        problem: "Найдите $\\mathrm{ord}_{5}(2)$.",
        solution:
          "$2^{1}\\equiv 2$, $2^{2}\\equiv 4\\equiv-1$, $2^{3}\\equiv-2\\equiv 3$, $2^{4}\\equiv 1\\pmod{5}$. Наименьший показатель $4=\\varphi(5)$: первообразный корень.",
      },
      {
        title: "Порядок степени",
        problem: "$\\mathrm{ord}_{7}(2)=3$. Чему равен $\\mathrm{ord}_{7}(2^{2})=\\mathrm{ord}_{7}(4)$?",
        solution:
          "$\\gcd(2,3)=1$, поэтому порядок тот же: $3/1=3$. Проверка: $4^{1}\\equiv 4$, $4^{2}\\equiv 2$, $4^{3}\\equiv 1\\pmod{7}$.",
      },
    ],
    sample: {
      id: "s",
      type: "open",
      prompt: "$a^{k}\\equiv 1\\pmod{n}$ влечёт, что порядок делит $k$. Если $\\mathrm{ord}_{n}(a)=4$ и $a^{12}\\equiv 1$, то $4$ делит $12$. Запишите $12/4=$",
      accepted: ["3"],
      explanation: "Частное показателей.",
      solution:
        "Свойство делимости порядка: $4\\mid 12$, частное $3$.",
    },
    problems: [
      {
        id: "p1",
        type: "choice",
        prompt: "Порядок $a$ по модулю $n$ определён, когда",
        options: [
          "$(a,n)=1$",
          "$n\\mid a$",
          "$a=0$",
          "$n$ составное обязательно",
        ],
        answerIndex: 0,
        explanation: "Класс должен быть единицей кольца $\\mathbb{Z}/n\\mathbb{Z}$.",
      },
      {
        id: "p2",
        type: "open",
        prompt: "$\\mathrm{ord}_{5}(4)$: $4^{1}\\equiv 4$, $4^{2}\\equiv 1\\pmod{5}$. Порядок равен",
        accepted: ["2"],
        explanation: "$4\\equiv-1$, $(-1)^{2}=1$.",
      },
      {
        id: "p3",
        type: "choice",
        prompt: "$a^{k}\\equiv 1\\pmod{n}$ равносильно тому, что",
        options: [
          "$k=\\varphi(n)$",
          "$\\mathrm{ord}_{n}(a)\\mid k$",
          "$k\\mid a$",
          "$k=0$",
        ],
        answerIndex: 1,
        explanation: "Основное свойство порядка в конечной группе.",
      },
      {
        id: "p4",
        type: "open",
        prompt: "По Эйлеру порядок делит $\\varphi(n)$. Для $n=9$ имеем $\\varphi(9)=6$. Возможный порядок $2$ по модулю $9$: $2^{1}=2$, $2^{2}=4$, $2^{3}\\equiv 8\\equiv-1$, $2^{6}\\equiv 1$. Порядок равен",
        accepted: ["6"],
        explanation: "Первообразный корень по модулю $9$.",
      },
      {
        id: "p5",
        type: "open",
        prompt: "$\\mathrm{ord}_{n}(a^{k})=\\mathrm{ord}(a)/\\gcd(k,\\mathrm{ord}(a))$. Если порядок $a$ равен $12$ и $k=8$, $\\gcd(8,12)=4$, новый порядок равен $12/4=$",
        accepted: ["3"],
        explanation: "Формула порядка степени.",
      },
      {
        id: "p6",
        type: "choice",
        prompt: "Элемент порядка $\\varphi(n)$ называют",
        options: [
          "квадратичным вычетом",
          "первообразным корнем",
          "числом Мерсенна",
          "единицей только при $a=1$",
        ],
        answerIndex: 1,
        explanation: "Образующая группы единиц.",
      },
    ],
    sources: src,
  }),

  makeLesson("uni-nt", 16, "Суммы двух квадратов", {
    purpose:
      "Представление $n=x^{2}+y^{2}$ связывает квадратичные вычеты (символ $\\bigl(\\frac{-1}{p}\\bigr)$) с мультипликативными тождествами и разложением на простые.",
    nonexample: {
      title: "Не всякое простое и не всякое $n$ есть сумма двух квадратов",
      text: "Простое $p\\equiv 3\\pmod{4}$ (например $3,7,11$) нельзя записать как $x^{2}+y^{2}$ с целыми $x,y$, кроме тривиального отказа. Число $6=2\\cdot 3$ несёт простой $3\\equiv 3\\pmod{4}$ в нечётной степени — тоже не сумма двух квадратов. При этом $9=0^{2}+3^{2}$ допустимо: показатель $3$ чётен.",
    },
    theory:
      "Тождество Брахмагупты–Эйлера $(a^{2}+b^{2})(c^{2}+d^{2})=(ac-bd)^{2}+(ad+bc)^{2}$ показывает, что множество сумм двух квадратов замкнуто относительно умножения. Простое $2=1^{2}+1^{2}$. Нечётное простое $p$ представимо в виде $p=x^{2}+y^{2}$ (с точностью до знаков и порядка — единственным образом) тогда и только тогда, когда $p\\equiv 1\\pmod{4}$; это равносильно $\\bigl(\\frac{-1}{p}\\bigr)=1$. Натуральное $n$ есть сумма двух целочисленных квадратов тогда и только тогда, когда в каноническом разложении всякий простой $p\\equiv 3\\pmod{4}$ входит в чётной степени. Доказательство «только тогда»: по модулю $4$ квадрат сравним с $0$ или $1$, сумма — с $0,1,2$, но не с $3$; для простых $p\\equiv 3\\pmod{4}$ используют стабильность $-1$ как невычета. Число представлений $r_{2}(n)$ связано с числом делителей $n$ в прогрессиях $1$ и $3$ по модулю $4$.",
    examples: [
      {
        title: "Простое $5$",
        problem: "Представьте $5$ суммой двух квадратов. Верно ли $5\\equiv 1\\pmod{4}$?",
        solution:
          "$5=1^{2}+2^{2}$ и $5\\equiv 1\\pmod{4}$. Других представлений с $x\\geqslant 0$, $y\\geqslant 0$, $x\\leqslant y$ нет.",
      },
      {
        title: "Произведение",
        problem: "$5=1^{2}+2^{2}$ и $13=2^{2}+3^{2}$. Найдите представление $65=5\\cdot 13$.",
        solution:
          "Тождество: $(1\\cdot 2-2\\cdot 3)^{2}+(1\\cdot 3+2\\cdot 2)^{2}=(-4)^{2}+7^{2}=16+49=65$. Иначе $(1\\cdot 3-2\\cdot 2)^{2}+(1\\cdot 2+2\\cdot 3)^{2}=1+8^{2}=65$.",
      },
    ],
    sample: {
      id: "s",
      type: "open",
      prompt: "$13=2^{2}+3^{2}$. Меньший из квадратов равен",
      accepted: ["4"],
      explanation: "$2^{2}=4$, $3^{2}=9$.",
      solution:
        "$13\\equiv 1\\pmod{4}$, представление $4+9$.",
    },
    problems: [
      {
        id: "p1",
        type: "choice",
        prompt: "Нечётное простое есть сумма двух квадратов тогда и только тогда, когда оно",
        options: [
          "$\\equiv 3\\pmod{4}$",
          "$\\equiv 1\\pmod{4}$",
          "равно $9$",
          "делит $n!$ при большом $n$",
        ],
        answerIndex: 1,
        explanation: "Теорема Ферма–Эйлера; $\\bigl(\\frac{-1}{p}\\bigr)=1$.",
      },
      {
        id: "p2",
        type: "open",
        prompt: "$2=1^{2}+1^{2}$. Сумма квадратов равна",
        accepted: ["2"],
        explanation: "Единственное чётное простое.",
      },
      {
        id: "p3",
        type: "choice",
        prompt: "Число $n$ — сумма двух квадратов, если в разложении $n$ все простые $\\equiv 3\\pmod{4}$ имеют показатели",
        options: ["нечётные", "чётные", "равные $1$", "равные $3$"],
        answerIndex: 1,
        explanation: "Критерий по каноническому разложению.",
      },
      {
        id: "p4",
        type: "open",
        prompt: "$10=1^{2}+3^{2}$. Больший квадрат равен",
        accepted: ["9"],
        explanation: "$1+9=10$; $10=2\\cdot 5$, оба допустимы.",
      },
      {
        id: "p5",
        type: "choice",
        prompt: "Тождество $(a^{2}+b^{2})(c^{2}+d^{2})=$",
        options: [
          "$(ac-bd)^{2}+(ad+bc)^{2}$",
          "$(a+c)^{2}+(b+d)^{2}$",
          "$a^{2}c^{2}+b^{2}d^{2}$",
        ...
        answerIndex: 0,
        explanation: "Мультипликативность нормы в гауссовых целых.",
      },
      {
        id: "p6",
        type: "open",
        prompt: "Простое $7\\equiv 3\\pmod{4}$ не есть сумма двух квадратов. Сколько целых решений $(x,y)$ у $x^{2}+y^{2}=7$?",
        accepted: ["0"],
        explanation: "Квадраты по модулю $4$ не дают $3\\equiv 7\\pmod{4}$.",
      },
    ],
    sources: src,
  }),

  makeLesson("uni-nt", 17, "Дзета-функция и ряд обратных квадратов (обзор)", {
    purpose:
      "Эйлерово произведение уже связывало простые с $\\zeta(s)$. Базельская задача $\\sum n^{-2}=\\pi^{2}/6$ — мост к анализу и к значениям $\\zeta$ на чётных.",
    nonexample: {
      title: "Ряд при $s=1$ не задаёт конечного $\\zeta(1)$",
      text: "Гармонический ряд $\\sum 1/n$ расходится, поэтому нельзя подставлять $s=1$ в $\\sum n^{-s}$ как число. Равенство $\\zeta(-1)=-1/12$ относится к аналитическому продолжению, а не к сумме натурального ряда $1+2+3+\\cdots$. Обзор ниже — про $\\mathrm{Re}\\,s>1$ и классическое $\\zeta(2)$.",
    },
    theory:
      "Дзета-функция Римана $\\zeta(s)=\\sum_{n=1}^{\\infty}n^{-s}$ абсолютно сходится при $\\mathrm{Re}\\,s>1$ и допускает эйлерово произведение $\\zeta(s)=\\prod_{p}(1-p^{-s})^{-1}$ (единственность разложения на простые). Полюс в $s=1$ соответствует расходимости гармонического ряда и влечёт расходимость $\\sum_{p}1/p$. Базельская задача: $\\zeta(2)=\\sum_{n=1}^{\\infty}n^{-2}=\\pi^{2}/6$ (Эйлер; позже — ряды Фурье функции $x^{2}$). Вообще $\\zeta(2k)$ есть рациональное кратное $\\pi^{2k}$ (числа Бернулли). Нечётные аргументы сложнее: $\\zeta(3)$ иррационально (Апери). Из произведения $\\zeta(s)\\sum_{n}\\mu(n)n^{-s}=1$ при $\\mathrm{Re}\\,s>1$. Частичные суммы $H_{n}^{(2)}\\to\\pi^{2}/6$; уже $1+1/4+1/9=1{,}361\\ldots$ близки к $1{,}645\\ldots$. Связь с простыми: логарифмическая производная $-\\zeta'/\\zeta=\\sum\\Lambda(n)n^{-s}$.",
    examples: [
      {
        title: "Первые слагаемые",
        problem: "Вычислите $1+1/4+1/9$ и сравните с $\\pi^{2}/6\\approx 1{,}645$.",
        solution:
          "$1+0{,}25+0{,}1\\overline{1}=1+1/4+1/9=1{,}361\\ldots$. Остаток ряда положительный, сумма строго больше.",
      },
      {
        title: "Эйлерово произведение при $s=2$",
        problem: "Почему $\\zeta(2)\\cdot(1-1/4)=\\sum_{k\\geqslant 0}(2k+1)^{-2}$?",
        solution:
          "Множитель $(1-2^{-2})$ вычёркивает чётные: остаётся сумма по нечётным знаменателям. Это шаг доказательства Эйлера через синус.",
      },
    ],
    sample: {
      id: "s",
      type: "open",
      prompt: "$\\zeta(2)=\\pi^{2}/6$. Запишите знаменатель рационального множителя при $\\pi^{2}$",
      accepted: ["6"],
      explanation: "Базельская задача.",
      solution:
        "Эйлер: сумма обратных квадратов равна $\\pi^{2}/6$.",
    },
    problems: [
      {
        id: "p1",
        type: "choice",
        prompt: "Ряд $\\sum n^{-s}$ абсолютно сходится при",
        options: [
          "$\\mathrm{Re}\\,s>1$",
          "$s=1$",
          "$\\mathrm{Re}\\,s<0$",
        ...
        answerIndex: 0,
        explanation: "Признак сравнения с интегралом / $p$-ряды.",
      },
      {
        id: "p2",
        type: "open",
        prompt: "Третий член ряда обратных квадратов $1/3^{2}$ в несократимом виде имеет знаменатель",
        accepted: ["9"],
        explanation: "$1/9$.",
      },
      {
        id: "p3",
        type: "choice",
        prompt: "Эйлерово произведение $\\zeta(s)=\\prod_{p}(1-p^{-s})^{-1}$ опирается на",
        options: [
        ...
        answerIndex: 0,
        explanation: "Каждый $n$ — произведение простых степеней ровно одним способом.",
      },
      {
        id: "p4",
        type: "choice",
        prompt: "$\\zeta(2k)$ при натуральном $k$ есть",
        options: [
          "трансцендентное не связанное с $\\pi$",
          "рациональное кратное $\\pi^{2k}$",
          "всегда $1$",
          "равно $\\zeta(3)$",
        ],
        answerIndex: 1,
        explanation: "Формула Эйлера через числа Бернулли.",
      },
      {
        id: "p5",
        type: "open",
        prompt: "$1+1/4=$ (первый плюс второй член $\\zeta(2)$). Запишите несократимую дробь $5/4$ как десятичную с точкой: $1.25$, либо $5/4$.",
        accepted: ["5/4", "1.25", "1,25"],
        explanation: "$1+1/4=5/4$.",
      },
      {
        id: "p6",
        type: "open",
        prompt: "При $s=1$ гармонический ряд расходится, поэтому конечного $\\zeta(1)$ у ряда нет. Запишите $1$ как показатель, при котором ряд расходится",
        accepted: ["1"],
        explanation: "Полюс дзета-функции.",
      },
    ],
    sources: src,
  }),
